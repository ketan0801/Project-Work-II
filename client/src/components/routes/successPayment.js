...
export const SuccessPayment = () => {
    ...
    const pdfRef = useRef(); // ✅ added

    const handleDownloadPDF = () => {
        const opt = {
            margin:       0.5,
            filename:     `Order_${paymentResponse.order_id}.pdf`,
    ...
    if (paymentResponse.error) return <GenericErrorMsg/>;
    if (!shippingAddressForm) return <BadRequest/>;
    if (!paymentResponse.hasOwnProperty("order_id")) return null;
    ...
    const renderShippingAddress = () => {
        ...
        return shippingAddressAttributes.map((value) => (
            <Grid key={value} item>{value}</Grid>
        ));
    }
    ...
    const renderShoppingProducts = () => {
        ...
        for (const [id, qty] of Object.entries(addToCart.productQty)) {
            ...
            products.push(
                <Grid key={id} container spacing={2} style={{ paddingTop: "2rem" }}>
                    <Grid item>
                    ...
                    <Grid item container xs={5} direction="column" style={{ fontWeight: "bold" }}>
                        ...
                        <Grid item>{product.productBrandCategory.type}</Grid>
                        <Grid item>{`Qty: ${qty} X ${product.price} = ${product.price * qty}`}</Grid>
                    </Grid>
                </Grid>
            );
        }
    ...
    return (
        <Grid container justify="center">
            <Grid ref={pdfRef} item xs={8} container spacing={2} style={{
